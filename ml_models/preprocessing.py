...
import pandas as pd
from utils.minio_helper import download_file, upload_file
...
def run() -> None:
    ...
    # Decoupled Cloud-Native Logic
    if os.getenv("MINIO_ENDPOINT"):
        input_path = download_file("raw/enriched_products.json", project_root / "data" / "processed" / "enriched_products.json")
        output_path = project_root / "data" / "processed" / "ml_ready_data.csv"
    else:
    ...
    try:
        ...
        if os.getenv("MINIO_ENDPOINT"):
            upload_file(output_path, "processed/ml_ready_data.csv")
